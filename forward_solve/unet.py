...
save_results_dir = './test/trial_' + str(trial) + '/'
...
K = 40; pdel2 = 0.05; pdeinf = 0.05; nbl = 1; nel = 0.1; pl = 1e-06; dircl = 100; uxypen = 0; ppow = 1;

print('\nK: {}, pdeinf: {}, pdel2: {}, nbl: {}, nel: {}, pl: {}, dircl: {}, ppow: {}, uxypen: {}, e_number: {}'.format(K, pdeinf, pdel2, nbl, 
                                                                                                                       nel, pl, dircl, 
                                                                                                                       ppow, uxypen, e_number), flush=True)
print('\n', flush=True)
print('\n', flush=True)
...
def pde_loss(xy, s, sx, sy, K, pdeinf, pdel2):
    ...
    loss_uxpen = uxypen * tf.reduce_mean(tf.math.pow(tf.abs(ux/h), ppow))
    loss_uypen = uxypen * tf.reduce_mean(tf.math.pow(tf.abs(uy/h), ppow))
    loss_uxxpen = uxypen * tf.reduce_mean(tf.math.pow(tf.abs(uxx), ppow))
    loss_uyypen = uxypen * tf.reduce_mean(tf.math.pow(tf.abs(uyy), ppow))
